...
한 좌석에 대한 동시 예매 경쟁을 5가지 전략으로 제어·비교하는 포트폴리오 프로젝트.

### 핵심 가치 세 가지

1. **동시성 제어.** 한 좌석 경쟁을 제어하고, 전략별로 비교하고, 왜 그렇게 했는지
   설명할 수 있어야 한다. 동시성 코드의 정확성과 설명 가능성은 타협하지 않는다.
2. **실제 서비스해도 괜찮을 퀄리티.** 데모용 눈속임이 아니라 실사용에 내놔도 되는
   수준을 목표로 한다. 화면이 깨지지 않고, 실패 경로가 처리돼 있고, 눌러야 하는
   버튼이 눌리고, 지난 내역이 남고, 표기가 실제 서비스 관례를 따른다.
   "동작은 하는데 이상하다"는 것도 결함으로 취급한다.
3. **기능 추가.** 실제 예매 서비스에 있어야 할 기능을 계속 채운다.

셋이 충돌하면 **동시성 정확성이 먼저다.** 그리고 새 기능이 기존 품질을 떨어뜨리면
기능을 미룬다 — 2번이 3번의 전제 조건이다.
