-- 공연 하나에 붙는 실제 예매처 목록(KOPIS 상세의 relates). 인터파크 같은 대형
-- 예매처뿐 아니라 공연장 자체 예매 페이지도 온다. 이 사이트의 회차·좌석·가격은
-- SeatGenerator 가 만든 추정치라, 상세 화면은 이 목록으로 "진짜 예매 가능한 곳"을
-- 안내한다 — KOPIS 상세 페이지 링크보다 한 단계 더 정확하다.
--
...
alter table booking_link
    add constraint fk_booking_link_performance foreign key (performance_id) references performance;
