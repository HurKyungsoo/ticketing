...
public class LocalDataSeeder implements CommandLineRunner {
    ...
    private void seedPerformances() {
        ...
        // 좌석도가 지정된 나머지 세 홀. 무대를 감싸는 구조(롯데), 한 층이 A~H 로 갈리는
        // 구조(세종), 좌/우 사이드석이 있는 소극장 구조(리사이틀홀)를 로컬에서 눈으로
        // 확인하려면 회차가 있어야 한다.
        ...
        seedPerformance("SEED-CLASSIC-3", "피아노 독주회", "클래식", "예술의전당 리사이틀홀", 354, 50_000);

        log.info("로컬 시드 공연 6개 생성 완료 (공연당 회차 {}개)", SCHEDULE_COUNT);
    }
