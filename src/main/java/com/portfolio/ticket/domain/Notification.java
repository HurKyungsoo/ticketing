 ...
 * 사용자에게 보여줄 알림 한 건.
 *
 * <p>지금은 "찜한 공연에 예매 가능한 회차가 열렸다" 하나뿐이지만, 나중에 종류가 늘 것을
 * 감안해 {@link NotificationType} 으로 구분한다.
 *
 ...
 * 으로 해석한다(WishlistService 가 중복 찜을 다루는 방식과 같다).
 *
 * <p>그 결과 한 공연당 한 사람에게 한 번만 간다. 8월 회차가 다 지난 뒤 9월 회차가 열려도
 * 다시 알리지 않는다 — 재알림의 가치보다 매일 같은 알림이 쌓이는 손해가 크다고 봤다.
 * 재알림이 필요해지면 유니크 키에 "어느 기간이 열렸는지"를 넣어 좁히면 된다.
 */
@Entity
@Table(
    name = "notification",
