...
구역이 무대를 기준으로 어디에 놓이는지는 `position` 으로 적는다(`CENTER` 기본 /
`LEFT` / `RIGHT` / `REAR`). 화면은 무대 뒤 구역을 무대 위쪽에 줄 순서를 뒤집어 그리고
(1열이 무대에서 가장 가까우므로 블록 아래쪽이 무대와 맞닿아야 한다), 정면 좌·우 구역은
중앙 구역 양옆에 나란히 놓는다. 위치는 **좌석 테이블에 넣지 않았다** — 구역 단위로 항상
같은 값이라 중복이고, 위치를 고치려면 좌석 수만 개를 다시 만들어야 한다. 좌석도가 아는
사실이므로 조회 시점에 붙인다.

**좌석도에 실존 홀을 넣을 때의 원칙**: 공식 자료로 확인한 수치만 쓰고 근사한 부분은
주석으로 남긴다. 예술의전당은 층별 총계까지만 공개하고 줄별 좌석 수는 공개하지 않으므로,
구역 합계는 공식 수치에 정확히 맞추고 그 합계를 줄로 나누는 분포만 우리가 정했다.
장애인석(콘서트홀 24석 / 리사이틀홀 4석)은 휠체어 공간이라 일반 좌석과 성격이 달라
제외했고, 공식 총계와의 차이는 전부 이 제외분이다.
...
- [x] 좌석도 무대 기준 배치 — 구역에 `position`(CENTER/LEFT/RIGHT/REAR)을 주면 화면이 무대를 기준으로 놓인다. 무대 뒤 구역(합창석)은 무대 위쪽에 줄 순서를 뒤집어 그리고, 빈야드형 홀의 좌우 구역은 정면 구역 양옆에 나란히 놓인다. 표시 순서도 구역명 사전순이 아니라 좌석도에 적은 순서를 따르므로 `"1층" → "1층뒤"` 같은 이름 트릭이 없어졌다
