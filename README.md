...
| 형태 | 용도 |
|---|---|
| `rows` + `seatsPerRow` | 줄 길이가 같은 직사각형 구역 (발코니 등) |
| `seatCounts: [30, 31, …]` | 줄별 좌석 수. 뒤로 갈수록 넓어지는 **부채꼴** |

부채꼴이 그대로 그려지는 건 화면이 줄을 가운데 정렬하기 때문이다(`.row { margin-inline: auto }`).
줄 길이만 주면 렌더러는 손댈 필요가 없다.

통로("N석마다")는 줄 길이가 대체로 같은 구역에만 넣는다. 줄이 30~43석으로 변하는
구역에 같은 규칙을 쓰면 줄마다 통로 위치가 달라지고, 줄이 가운데 정렬이라 세로로
어긋나 지그재그로 보인다. 실제 통로 위치는 구역을 좌/중앙/우 블록으로 쪼개야
표현할 수 있고 아직 미지원이다.
...
- [ ] 좌석도 좌/중앙/우 블록 — 같은 층 안에서 구역을 가로로 나란히 놓아 실제 통로 위치와 구역별 등급을 표현. 현재는 구역이 위아래로만 쌓인다
