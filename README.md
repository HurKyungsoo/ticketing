...
|---|---|---|
| 예술의전당 콘서트홀 (2,505석) | 층별 총계, 합창석 274, 1층 블록 구분(A~E) | 블록별 좌석 수, 줄별 분포, 등급 |
...
근거를 어디서 얻었는지도 남겨둔다. 예술의전당 콘서트홀의 블록 구분은 공식 홈페이지 본문이
아니라 전당이 배포하는 **좌석배치도 파일**에 들어 있다 — "휠체어석의 경우 가용 가능 휠체어
수량은 A블럭, E블럭 각각 6대씩 입장이 가능합니다"라는 안내로 A·E 가 양끝임을 알 수 있고,
같은 파일에 B·D·F·H·N 블록 라벨과 1층 / 2·3층 두 장의 배치도가 있어 1층이 A~E,
2·3층이 F~N 계열임이 확인된다(2·3층 경계는 못 뽑아내 층당 한 구역으로 뒀다).
